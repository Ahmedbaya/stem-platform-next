...
interface OrganizerCompetitionsViewProps {
  competitions: Competition[]
  userId: string
}

export function OrganizerCompetitionsView({ competitions, userId }: OrganizerCompetitionsViewProps) {
  const [isCreateDialogOpen, setIsCreateDialogOpen] = useState(false)

  // Filter competitions by status
  const draftCompetitions = competitions.filter((comp) => comp.status === "draft" && comp.organizerId === userId)
  const publishedCompetitions = competitions.filter(
    (comp) => comp.status === "published" && comp.organizerId === userId,
  )
  const ongoingCompetitions = competitions.filter((comp) => comp.status === "ongoing" && comp.organizerId === userId)
  const completedCompetitions = competitions.filter(
    (comp) => comp.status === "completed" && comp.organizerId === userId,
  )
              ...
              <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                ...
                {draftCompetitions.length === 0 && (
                  <div className="col-span-full text-center py-12 text-muted-foreground">
                    No draft competitions. Click &quot;Create Competition&quot; to get started.
                  </div>
                )}
              </div>
            </TabsContent>
